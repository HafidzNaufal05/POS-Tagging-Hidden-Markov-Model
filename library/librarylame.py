...
def AddDictionaryPrefix():
    ...
    f = open("data/dataset/prefix.csv","w+")
    for word in fs:
        word = word.rstrip("\n")
        l_tag = []
...
def PartitionData(param_partisi,param2):
    ...
    file_location = []
    for x in fs:
        if(i % int(l/(param_partisi[0]-1)) == 0 or i >= int(l / param_partisi[0]) * param_partisi[0]):
            if i >= int(l / param_partisi[0]) * param_partisi[0]:
                f1 = open("data/dataproses/data_partisi_"+ str(param_partisi[0]) +".csv","a")
            else:
